...
public class MethodTrapezeRule extends AbstractMethod {

    int printAfter;

    public MethodTrapezeRule(int printAfter) {
    ...
    public Matrix integrate(Matrix A, Matrix B, Matrix x0, Matrix r, double T, double interval, boolean rIsConstant, boolean calculateError, String task){
        ...
        I.identityMatrix();
        Matrix P=I.substract(A.multiply(T/2));
        P=P.inverse();
        Matrix pom=P.clone();
        Matrix R=P.multiply(I.add(A.multiply(T/2)));
        Matrix S=pom.multiply(B.multiply(T/2));
        ...
        for (int step = 1; step <= integrationSteps; step++) {
            ...
            Matrix rNext=new Matrix(2,1);

            if (!rIsConstant) {
                r.setElement(0, 0, T*step);
                r.setElement(1, 0, T*step);

                rNext.setElement(0, 0, T*(step+1));
                rNext.setElement(1, 0, T*(step+1));
            }
            else{
                rNext=r.clone(); // ako je r konst
            }

            xk=R.multiply(xk).add(S.multiply(r.add(rNext)));
            ...
            //spremanje u file
//            String filename = "./src/main/java/LAB5/outputFiles/" + task + "Trapeze.txt";
